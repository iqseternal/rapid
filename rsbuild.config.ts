...
import mainRspackConfig from './apps/desktop-node/rspack.config';
import rendererRsbuildConfig from './apps/desktop-web/rsbuild.config';

import treeKill from 'tree-kill';
import chokidar from 'chokidar';

const bin = resolve(__dirname, './node_modules/.bin/electron');
if (!existsSync(bin) || !statSync(bin).isFile()) {
  throw new Error(`未在node_modules中找到electron, 请检查依赖是否安装, pnpm install ?`);
}
...
;(async () => {
  const { content } = await loadConfig({
    cwd: join(__dirname, './apps/desktop-web/'),
    envMode: 'production',
    path: join(__dirname, './apps/desktop-web/rsbuild.config.ts'),
  });
