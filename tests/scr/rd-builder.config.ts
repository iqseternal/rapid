...
import { DIRS, rules } from '../../src/rd-builder';
...
export default defineConfig({
  dirs: {
    ...
    browser: {
      entry: join(DIRS.ROOT_DIR, './src/rd/code/browser/index.tsx'),
      output: join(DIRS.ROOT_DIR, './.rd-packer/out/renderer')
    },

    sandbox: {
      entry: join(DIRS.ROOT_DIR, './src/rd/code/sandbox/index.ts'),
      output: join(DIRS.ROOT_DIR, './.rd-packer/out/preload')
    },
    main: {
      entry: join(DIRS.ROOT_DIR, './src/rd/app.ts'),
      output: join(DIRS.ROOT_DIR, './.rd-packer/out/main')
    }
  },
  transformerBrowserRsbuildConfig: () => {



  },
  transformerMainRspackConfig: () => {


  },
  transformerSandboxRspackConfig: () => {
